import type { EscolhaAutomaticaRow } from "@/lib/latam/escolhaAutomatica";
...
type PdfGroup = { name: string; login: string; rows: PdfRow[] };

const COLS = [
  { key: "id", title: "ID", w: 90 },
  { key: "nome", title: "Cedente", w: 150 },
  { key: "cpf", title: "CPF", w: 86 },
  { key: "pax", title: "Livres", w: 48 },
  { key: "lim", title: "Limite", w: 48 },
  { key: "clube", title: "Clube", w: 72 },
] as const;
...
function drawRow(c: PdfCanvas, x: number, y: number, row: PdfRow, alt: boolean) {
  c.rect({
    x,
    y,
    w: TABLE_W,
...
function groupRows(rows: EscolhaAutomaticaRow[]): PdfGroup[] {
  const map = new Map<string, PdfGroup>();
  for (const r of rows) {
    const key = r.owner.id || r.owner.login;
    const cur = map.get(key) || {
      name: r.owner.name || "Sem funcionario",
      login: r.owner.login || "—",
      rows: [],
    };
    cur.rows.push({
      ...
      cpf: r.cedente.cpf,
      cpfFree: r.account.cpfFree,
      cpfLimit: r.account.cpfLimit,
      clubLabel: r.clubLabel === "cancelado" ? "Cancelado" : "Sem clube",
    });
    map.set(key, cur);
  }
...
export function buildEscolhaAutomaticaPdf(input: {
  monthLabel: string;
  minFree: number;
  rows: EscolhaAutomaticaRow[];
}) {
  ...
  function footer() {
    canvas.text({
      text: `Escolha automatica LATAM  ·  ${input.monthLabel}  ·  pagina ${pageNo}`,
      x: MARGIN,
      y: PAGE_HEIGHT - 22,
      size: 8,
      color: C.muted,
  ...
  canvas.rect({ x: 0, y: 78, w: PAGE_WIDTH, h: 4, fill: C.teal });
  canvas.text({
    text: "Vias Aereas",
    x: MARGIN,
    y: 28,
  ...
  canvas.text({
    text: `Mais de ${input.minFree} pax livres  ·  sem clube ativo/inativo  ·  nao transferido  ·  sem cancelamento no mes se estava transferido  ·  ${input.monthLabel}`,
    x: MARGIN,
    y: 68,
    size: 8,
  ...
  const cardW = (PAGE_WIDTH - MARGIN * 2 - 8) / 2;
  const cards = [
    { label: "Contas", value: String(totalContas), fill: C.tealBg, accent: C.teal },
    { label: "Passageiros livres", value: String(totalPax), fill: C.violetBg, accent: C.violet },
  ];
  ...
  groups.forEach((g, gi) => {
    ...
    canvas.text({
      text: `@${g.login}  ·  ${g.rows.length} contas  ·  ${pax} pax livres`,
      x: MARGIN + 16,
      y: y + 34,
      size: 8,
      color: C.muted,
    ...
    g.rows.forEach((row, i) => {
      ...
      drawRow(canvas, MARGIN, y, row, i % 2 === 1);
      y += ROW_H;
    });
    y += 14;
  });
