...
export async function GET(req: NextRequest) {
  ...
  if (mode === "usage") {
    ...
    if (programa === LoyaltyProgram.SMILES) {
      ...
      // janela móvel 365 dias (inclui o dia selecionado)
      windowEnd = endOfDayUTC(issuedDate);
      windowStart = addDaysUTC(new Date(Date.UTC(issuedDate.getUTCFullYear(), issuedDate.getUTCMonth(), issuedDate.getUTCDate(), 0, 0, 0, 0)), -364);
    } else {
      // sem regra por enquanto: mostra só o ano como janela "informativa"
      windowStart = startOfYearUTC(issuedDate);
      windowEnd = endOfYearUTC(issuedDate);
    }
