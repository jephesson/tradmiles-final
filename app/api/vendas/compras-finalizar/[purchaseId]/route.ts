...
export async function PATCH(req: Request) {
  ...
  if (!session?.id) return NextResponse.json({ ok: false, error: "Não autenticado" }, { status: 401 });

  const body = await req.json().catch(() => ({}));
  const purchaseId = String(body.purchaseId || "").trim();
  if (!purchaseId) return NextResponse.json({ ok: false, error: "purchaseId obrigatório" }, { status: 400 });

  try {
    const out = await prisma.$transaction(async (tx) => {
      const purchase = await tx.purchase.findUnique({
        where: { id: purchaseId },
        select: { id: true, status: true, points: true, totalCents: true, metaMilheiroCents: true, finalizedAt: true },
      });
      ...
      const avgMilheiroCents = soldPoints > 0 ? Math.round((pointsValueCents * 1000) / soldPoints) : 0;
