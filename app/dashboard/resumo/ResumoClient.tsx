...
export default function CedentesResumoClient() {
  ...
  const [rateLatam, setRateLatam] = useState("20");  // você ajusta
  const [rateSmiles, setRateSmiles] = useState("18");
  const [rateLivelo, setRateLivelo] = useState("22");
  const [rateEsfera, setRateEsfera] = useState("17");
  ...
  async function load() {
    ...
    try {
      ...
      // preenche caixa do dia com o latest
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border bg-white p-4 space-y-3">
        ...
        <div className="text-xs text-slate-600">
          Se quiser “automático no fim do dia”, dá pra colocar um Cron na Vercel chamando um endpoint (te monto isso depois).
        </div>
      </div>
    </div>
  );
