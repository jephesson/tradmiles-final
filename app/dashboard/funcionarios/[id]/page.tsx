...
export default function FuncionarioEditPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    ...
    try {
      ...
      const found = (json.data || []).find((x: FuncItem) => x.id === params.id);
  ...
  async function gerarLink() {
    ...
    setMsg("");
    try {
      const res = await fetch(`/api/funcionarios/${item.id}/invite`, { method: "POST", cache: "no-store" });
      const json = await res.json();
    ...
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">Gere o link de indicação para o funcionário.</p>
        </div>
        ...
        <div className="rounded-2xl border p-4 space-y-4 max-w-2xl">
          <div>
            ...
            <div className="text-sm text-slate-600">
              ...
              <span className="font-medium">{item.team}</span> • CPF: <span className="font-medium">{item.cpf || "-"}</span>
            </div>
          </div>
          ...
          <div className="rounded-2xl border p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-slate-500">Clique para gerar ou regenerar o link.</div>
              </div>

              <button onClick={gerarLink} className="rounded-xl bg-black px-4 py-2 text-white">
            ...
            {inviteUrl && (
              <div className="mt-2 text-xs text-slate-500">
                Abra este link em uma janela anônima para testar: <span className="font-mono">{inviteUrl}</span>
              </div>
            )}
          </div>
        </div>
