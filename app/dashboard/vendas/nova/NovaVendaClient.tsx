...
export default function NovaVendaClient({
  ...
  /** 1 = dados · 2 = cedentes · 3 = emissão (só uma seção visível) */
  ...
  // Popup biometria / acesso (LATAM e Smiles, após selecionar cedente)
  ...
  // Carrega credenciais ao abrir o wizard (LATAM / Smiles).
  useEffect(() => {
    if (!biometriaModalOpen || !sel?.cedente?.id) return;
  ...
  const canSave = useMemo(() => {
    ...
    if (program === "LATAM") {
      const pc = (purchaseCode || "").trim().toUpperCase();
      // Opcional se pulou o Order ID; se preencher, precisa ser LA…
      if (pc && !/^LA[A-Z0-9]+$/i.test(pc)) return false;
    }
    if (
      program === "SMILES" &&
  ...
  async function doSave() {
    ...
    if ((program === "LATAM" || program === "SMILES") && !latamEmissionUnlocked) {
      setBiometriaModalOpen(true);
      return alert(
        "Conclua o fluxo de acesso/biometria antes de salvar a emissão."
      );
    }
    ...
    if (program === "LATAM") {
      const pc = (purchaseCode || "").trim().toUpperCase();
      if (pc && !/^LA[A-Z0-9]+$/i.test(pc))
        return alert("Código de compra LATAM inválido (deve começar com LA).");
    }
    if (
      program === "SMILES" &&
      !/^[A-Z]{3}$/.test((departureAirportIata || "").trim().toUpperCase())
  ...
  const flowSteps = [
    { id: 1 as const, label: "Dados da venda" },
    { id: 2 as const, label: "Cedentes" },
    { id: 3 as const, label: "Emissão" },
  ];

  return (
    <div className="mx-auto max-w-[1800px] space-y-5 p-4 pb-10 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Nova venda</h1>
            <span className="rounded-full border border-slate-200 bg-white px-2.5 py-0.5 text-[11px] font-medium text-slate-500">
              {program} · {fmtInt(pointsTotal)} pts · {fmtInt(passengers)} PAX
            </span>
          </div>
          {sugError ? (
            <div
              className="mt-2 flex gap-2 rounded-xl border border-rose-200 bg-rose-50/90 px-3 py-2 text-xs text-rose-800"
              role="alert"
            >
              {sugError}
            </div>
      ...
      <nav
        ...
        className="flex flex-wrap gap-2 rounded-2xl border border-slate-200/80 bg-white p-2 shadow-sm"
      >
        {flowSteps.map((s) => {
          const active = flowStep === s.id;
          const done = flowStep > s.id;
          const locked = s.id === 3 && !sel;
          const blockedCedentes = s.id === 2 && !canGoToCedentes && flowStep < 2;
          const disabled = locked || blockedCedentes;
          return (
            <button
              key={s.id}
              type="button"
              ...
              onClick={() => {
                if (disabled) return;
                if (s.id === 3 && !sel) return;
                setFlowStep(s.id);
              }}
              className={cn(
                "inline-flex min-w-0 flex-1 items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs font-semibold transition sm:flex-none sm:min-w-[160px]",
                active
            ...
            >
              <span
                className={cn(
                  ...
                  active ? "bg-white/15 text-white" : "bg-white text-slate-700 border border-slate-200"
                )}
              >
                {s.id}
              </span>
      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 bg-white/70 px-5 py-3 sm:px-6">
          <button
            type="button"
          ...
          {sel ? (
            ...
            <span className="text-xs text-slate-500">Selecione um cedente para seguir</span>
          )}
        </div>
      </section>
      ) : null}
                ...
                <div className="p-5 sm:p-6">
                  <div className="mb-4 flex items-start gap-3">
                    ...
                    <div className="min-w-0 pt-0.5">
                      <h2 className="text-base font-semibold tracking-tight text-slate-900">
                        {program === "LATAM"
                          ? "Acesso, biometria e Order ID"
                          : "Acesso e código Smiles"}
                      </h2>
                      <p className="mt-1 text-xs leading-relaxed text-slate-500">
                        Conclua o popup com credenciais e código de verificação antes de
                        preencher a emissão.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4 text-sm text-amber-950">
                    <p>
                      O fluxo foi interrompido. Reabra o popup para continuar (ou pular
                      etapas, se precisar).
                    </p>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const next = e.target.value
                              .replace(/[^a-zA-Z0-9]/g, "")
                              .toUpperCase();
                            setPurchaseCode(next);
                            if (/^LA[A-Z0-9]+$/i.test(next)) {
                              setPagamentoLinkReady(buildLatamPagamentoLink(next));
                            } else if (!next) {
                              setPagamentoLinkReady(null);
                            }
                          }}
                          ...
                          <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-3">
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                              <button
                                type="button"
                                onClick={() =>
                                  copyText("Link de pagamento", pagamentoLinkReady)
                                }
