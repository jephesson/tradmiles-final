...
function clickMenorPreco() {
  for (const el of document.querySelectorAll("button, option, span, div, li, label")) {
    const t = (el.textContent || "").trim();
    if (/^(menor\s+pre[cç]o|mais\s+barato)$/i.test(t)) {
      el.click();
      return;
    }
  }
...
function isResultsPage() {
  return /selecao-de-voo/i.test(location.pathname || "");
}
...
function pageKind() {
  ...
  if (/n[aã]o\s+(foi\s+)?poss[ií]vel\s+encontrar|n[aã]o\s+encontramos\s+voo|nenhum\s+voo|sem\s+voo\s+dispon/i.test(t)) {
    return "NO_RESULTS";
  }
  if (flightCards().length > 0) return "RESULTS";
  if (/selecione\s+seu\s+voo|a\s+partir\s+de\s+R\$/i.test(t) && /\d{1,2}:\d{2}/.test(t)) return "RESULTS";
  return "WAIT";
}
...
async function run() {
  const t0 = Date.now();
  while (!isGolSearch() && Date.now() - t0 < 12000) await sleep(400);
  if (!isGolSearch()) {
  ...
  if (!isResultsPage()) {
    const waitResults = Date.now();
    while (!isResultsPage() && pageKind() === "WAIT" && Date.now() - waitResults < 40000) {
      await sleep(500);
    }
    if (!isResultsPage() && pageKind() === "WAIT") {
      sendResult({ ok: false, error: "A GOL não abriu a lista de voos." });
      return;
    }
  }
